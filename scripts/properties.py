...
class VatBakerProperties(bpy.types.PropertyGroup):
    ...
    include_position:       bpy.props.BoolProperty(name="Enable", default=True, description="Bake position VAT texture")
    include_rotation:       bpy.props.BoolProperty(name="Enable", default=True, description="Bake normal VAT texture")
    ...
    rotation_suffix:        bpy.props.StringProperty(name="Suffix", default="_VAT_Normal", description="Suffix that will be appended to the output file.")
    ...
    uv_channel:             bpy.props.IntProperty(name="UV Channel", default=1, min=0, soft_max=4, description="UV channel index to set the VAT UVs")
    position_is_hdr:        bpy.props.BoolProperty(name="Save as EXR", default=True, description="Save position data as .exr (HDR values) or .png (0..1 values). NOTE: Position LDR doesnt work yet!")
    rotation_is_hdr:        bpy.props.BoolProperty(name="Save as EXR", default=True, description="Save rotation data as .exr (HDR values) or .png (0..1 values).")
